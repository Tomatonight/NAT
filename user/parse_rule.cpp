...
void parse_str(std::string str, netlink_connect *connect)
{
    ...
    else if (str.find("start_snat") != std::string::npos)
    {
        std::cmatch match;
        uint32_t ip;
        std::regex reg("[ ]*start_snat[ ]+([^ ]+)[ \n]*");
        if (std::regex_match(str.data(), match, reg))
        {
            ip = ntohl(inet_addr(std::string(match[1]).data()));
        }
        else
        {
            printf("start_snat err\n");
            return;
        }
        msg.action = MSG_ACTION_START_SNAT;
        msg.sip = ip;
    }
    else if (str.find("end_snat") != std::string::npos)
    {
        msg.action = MSG_ACTION_STOP_SNAT;
    }
    else if (str.find("add_dnat") != std::string::npos)
    {
        std::cmatch match;
        uint32_t ip;
        std::regex reg("[ ]*add_dnat[ ]+([^ ]+)[ ]+([^ ]+)[ ]+([^ ]+)[ ]+([^ ]+)[ \n]*");
        if (std::regex_match(str.data(), match, reg))
        {
            msg.action=MSG_ACTION_ADD_DNAT;
            msg.sip=ntohl(inet_addr(std::string(match[1]).data()));
            msg.sport=(atoi(std::string(match[2]).data()));
            msg.dip=ntohl(inet_addr(std::string(match[3]).data()));
            msg.dport=(atoi(std::string(match[4]).data()));
        }
        else
        {
            printf("add_dnat err\n");
    ...
    else if (str.find("delete_dnat") != std::string::npos)
    {
       
        std::cmatch match;
        uint32_t ip;
        std::regex reg("[ ]*delete_dnat[ ]+([^ ]+)[ ]+([^ ]+)[ ]+([^ ]+)[ ]+([^ ]+)[ \n]*");
        if (std::regex_match(str.data(), match, reg))
        {
            msg.action=MSG_ACTION_DELETE_DNAT;
            msg.sip=ntohl(inet_addr(std::string(match[1]).data()));
            msg.sport=atoi((std::string(match[2]).data()));
            msg.dip=ntohl(inet_addr(std::string(match[3]).data()));
            msg.dport=atoi((std::string(match[4]).data()));
